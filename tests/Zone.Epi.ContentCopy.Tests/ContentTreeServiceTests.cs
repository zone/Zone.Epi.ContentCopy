...
namespace Zone.Epi.ContentCopy.Tests
{
	...
	public class ContentTreeServiceTests
	{
		...
		public void GetContentFamily_ValidRootId_ReturnsContentTreeRootItem()
		{
			...
			_stubContentLoader.GetChildren<PageData>(rootReference, testLanguage).Returns(new List<PageData> { childPage1, childPage2 });
			_stubContentLoader.GetChildren<PageData>(childReference1, testLanguage).Returns(new List<PageData> { new PageData() });
